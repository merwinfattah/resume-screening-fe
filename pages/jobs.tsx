...
export default function Jobs () {
    ...
    const [selectedIndexModal, setSelectedIndexModal] = useState<number|null>(null);
    const showModal = (id:number) => {
        setIsModalOpen(true);
        setSelectedIndexModal(id);
      };
                             ...
                             <>
                                <div>
                                    <div  className={`flex justify-between px-8 pb-[21px] pt-[22px] w-[1148px] h-[72px] z-10 mb-1 rounded-md ${open? ' rounded-b-none  drop-shadow-lg': ''} bg-light_neutral_200`}>
                                        <EditorInput initialValue={department.name} onValueChange={(newName) => handleNameChange(newName, department.createdDate)} />
                                        <div className={'flex gap-[18px] items-center'}>
                                            { positionDataList.filter( position => position.department === department.name && position.isTrash.isInTrash === false).length > 0? 
                                            (
                                            <>
                                    ...
                                    <Transition
                                        ...
                                        <Disclosure.Panel className={`w-[1148px] flex flex-col gap-[18px] justify-center pt-[32px] pb-[29px] z-0 bg-light_neutral_200 rounded-b-lg rounded-l-md ${open? 'rounded-t-none': ''}`} >
                                                {positionDataList.filter(position => position.department === department.name && position.isTrash.isInTrash !== true).map((position: any, index: number) => (
                                                    <div key={index} className={`flex justify-center gap-4`}>
                                                        <button className={` text-2xl text-semantic_red_500`} onClick={()=>showModal(position.id)}><BsFillTrashFill /></button>
                                                        {selectedIndexModal === position.id  && 
                                                        <Modal 
                                                        type='deletePosition'
                                                        isOpen={isModalOpen} 
                                                        onClose={closeModal} 
                                                        headline={`Delete "${position.position}" position on ${position.department} department?`}
                                                        content="You can still access this position on trash and will be deleted permanently after 30 days"
                                                        />
                                                        }
